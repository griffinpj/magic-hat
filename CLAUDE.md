...
Layered so responsibilities stay isolated:
...
- `Services/` — orchestration over models + clients.
  - `ImportService` — applies a parsed import (add/replace), writes audit.
  - `CardHydrationService` — lazily fetches metadata for visible cards.
- `Features/` — one folder per screen area (`Root`, `Collection`, `Decks`,
  `History`, `Search`).
