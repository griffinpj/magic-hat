...
struct DeckAddSortTests {
    ...
    @Test func ordersByEachKeyAndKeepsTiesInPlace() {
        ...
        func names(_ sort: DeckAddSort) -> [String] { sort.apply(rows, card: { $0 }).map(\.name) }
        ...
        #expect(DeckAddSort.priceHigh.scryfall.sort == .price && DeckAddSort.priceHigh.scryfall.direction == .descending)
    }
}
